...
const insertRev = async ({ dbs, _id, _rev, revHelpers, dataToUpdate }) => {
    var error
    try {
        var keyRev = {
            _id,
            _rev: revHelpers().nextRev({ _rev })
        }
...
export default async (data) => {
    const { _id, _rev, dbs, revHelpers, dataToUpdate, standarizedResponse } = data
